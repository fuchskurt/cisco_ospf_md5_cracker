    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.checkbox(&mut self.charset_letters, "Upper Case Letters");
                ui.checkbox(&mut self.charset_numbers, "Numbers 0-9");
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(egui::Slider::new(&mut self.key_max_len, 1..=32).suffix(" chars"));
            });

            let progress = self.shared_progress.lock().unwrap().clone();
            if !progress.is_empty() {
                ui.label(format!("Progress: {}", progress));
            }

            if self.running {
                ui.add_enabled(false, egui::Button::new("Cracking..."));
                if ui.button("Stop").clicked() {
                    self.stop_requested.store(true, Ordering::Relaxed);
                }
            } else if ui.button("Start Brute-force").clicked() && !self.started {
            ...
            if !self.running {
                self.results = self.shared_results.lock().unwrap().clone();
...
fn brute_force_recursive(
    ...
    for &c in charset.iter() {
        ...
        key[pos] = c;
        if let Some(found) = brute_force_recursive(
            ospf_packet,
            digest,
            charset,
            max_len,
...
fn run_crack(
    ...
    let max_len: usize = max_len
        .parse()
        .map_err(|_| "max_len must be integer".to_owned())?;
    ...
    let local_results: Result<Vec<OspfResult>, String> = ospf_packets
        .par_iter()
        .enumerate()
        .map(|(idx, pkt)| {
            ...
            let key = brute_force_parallel(
                Arc::new(pkt.ospf_packet.clone()),
...
fn main() {
    let app = OspfGuiApp {
        shared_results: Arc::new(std::sync::Mutex::new(Vec::new())),
